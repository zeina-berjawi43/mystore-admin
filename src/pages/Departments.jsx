...
export default function Departments() {
  ...
  const uploadImage = async (
    ...
    /*
      ...
      Department images are therefore
      stored in the category-images bucket.
    */
  ...
  const save = async (event) => {
    ...
    if (
      ...
      return alert(
        "Enter a valid whole-number order."
      );
    }
    ...
    try {
      ...
      const response =
        await authorizedFetch(
          editing
            ? `${API_URL}/departments/${editing._id}`
            : `${API_URL}/departments`,
          {
            method:
              editing
  ...
  const remove = async (
    ...
    if (
      !window.confirm(
        `Delete "${department.name}"? Departments with categories cannot be deleted.`
      )
    ) {
      return;
    }
