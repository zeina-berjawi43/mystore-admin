...
const getCustomerEmail = (customer) => customer?.email || "No email";

const getCustomerPhone = (customer) => customer?.phone || "No phone";
...
function Orders() {
  ...
  const fetchOrders = async (showLoading = true) => {
    try {
      ...
      const receivedOrders = Array.isArray(response.data)
        ? response.data
  ...
  /* ============================================================
     GET INVOICE ID / NUMBER / DATE
  ============================================================ */

  const getInvoiceId = (order) => {
    ...
    if (order.invoice && typeof order.invoice === "object") {
      return order.invoice._id || order.invoice.id || null;
    }
  ...
  const getInvoiceNumber = (order) => {
    ...
    if (order.invoice && typeof order.invoice === "object") {
      return order.invoice.invoiceNumber || order.invoice.number || "";
    }

    return "";
  };

  const getInvoiceDate = (order) => {
    if (!order) return "";
  ...
  const filteredOrders = orders.filter((order) => {
    ...
    const matchesInvoiceDate =
      !invoiceDate ||
      getLocalDateValue(getInvoiceDate(order)) === invoiceDate;
  ...
  const handleCloseModal = () => {
    if (updatingStatus || deletingOrder || creatingInvoice) {
      return;
    }
  ...
  return (
    <div className="orders-page">
      ...
      <div className="orders-header">
        <div>
          <h1>Orders</h1>
          <p>Manage customer orders and invoices</p>
        </div>
      ...
      <div className="orders-count">
        Showing <span>{filteredOrders.length}</span> of{" "}
                    ...
                    <td>
                      <div className="order-customer">
                        ...
                        <div>
                          ...
                          <small>{getCustomerEmail(customer)}</small>
                        </div>
                      </div>
                    </td>
          ...
          >
            <div className="order-modal-header">
              ...
              <button
                ...
                onClick={handleCloseModal}
                disabled={updatingStatus || deletingOrder || creatingInvoice}
              >
                ✕
              </button>
            </div>
            ...
            <div className="order-detail-section">
              ...
              <div className="order-detail-customer">
                <div className="order-avatar large">
                  {getInitials(selectedOrder.user || selectedOrder.customer)}
                </div>

                <div>
                  <strong>
                    {getCustomerName(
                      selectedOrder.user || selectedOrder.customer
                    )}
                  </strong>
            ...
            <div className="order-detail-section">
              ...
              <p className="shipping-address">
                {selectedOrder.shippingAddress || "No shipping address"}
              </p>
            </div>
              ...
              <div className="status-buttons">
                {ORDER_STATUSES.map((status) => (
                  <button
                    key={status}
                    type="button"
                    className={`status-change-button ${getStatusClass(
                      status
            ...
            <div className="order-detail-section">
              ...
              {getInvoiceId(selectedOrder) ? (
                <button
                  type="button"
                  className="view-invoice-button"
                  onClick={() =>
                    handleViewInvoice(getInvoiceId(selectedOrder))
                  }
                >
                  Open Invoice
                </button>
                ...
                <button
                  ...
                  className="create-invoice-button"
                  disabled={creatingInvoice}
                  onClick={() => handleCreateInvoice(selectedOrder._id)}
                >
            ...
            <div className="order-modal-footer">
              <button
                type="button"
                className="delete-order-button"
                disabled={deletingOrder || updatingStatus || creatingInvoice}
