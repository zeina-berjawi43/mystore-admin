...
const fetchProducts = async (showInitialLoading = false) => {
try {
...
try {
  ...
  // IMPORTANT:
  // The backend requires a valid image type.
...
try {
  ...
  setProducts((previousProducts) =>
    previousProducts.map((item) =>
      item._id === product._id ? { ...item, price } : item
    )
  );
} catch (error) {
...
const handleDiscountUpdate = async (product, newDiscount) => {
...
try {
  ...
  setProducts((previousProducts) =>
    previousProducts.map((item) =>
      item._id === product._id ? { ...item, discount } : item
    )
  );
} catch (error) {
...
try {
  setUpdatingAvailability((previous) => ({
    ...previous,
    [product._id]: true,
  }));
...
const brand = brands.find((item) => item._id === product.brand);
...
return ( <div className="products-page"> <div className="products-loading"> <div className="products-spinner" /> <p>Loading products...</p> </div> </div>
);
}
...
return ( <div className="products-page"> <div className="products-header"> <div> <h1>Products</h1> <p>Manage your products, prices and discounts.</p> </div>
  ...
  {error && (
    <div className="products-error">
      ...
      <button type="button" onClick={() => fetchProducts(false)}>
        Try Again
      </button>
    </div>
  )}
              ...
              <tr key={product._id}>
                ...
                <td>
                  <input
                    ...
                    onBlur={(event) =>
                      handlePriceUpdate(product, event.target.value)
                    }
                    onKeyDown={(event) => {
                      if (event.key === "Enter") {
                        event.target.blur();
                ...
                <td>
                  <div className="discount-input-wrapper">
                    <input
                      ...
                      max="100"
                      step="1"
                      disabled={updatingDiscount[product._id]}
                      onBlur={(event) =>
          ...
          <div className="form-group">
            ...
            <label
              ...
              className={`image-upload-box ${
                isDraggingImage ? "image-upload-dragging" : ""
            ...
            {editingProduct && !imageFile && (
              <small className="current-image-text">
                Current image is already uploaded. Drag & drop or
                choose another image only if you want to replace
                it.
              </small>
            )}
          </div>
          ...
          <div className="modal-availability-row">
            ...
            <button
              ...
              className={`availability-switch modal-switch ${
                form.availability ? "switch-on" : "switch-off"
              }`}
              onClick={() =>
                setForm((previous) => ({
                  ...previous,
